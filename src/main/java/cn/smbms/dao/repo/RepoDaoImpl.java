...
public class RepoDaoImpl implements RepoDao {
    ...
    getSecretList(Connection connection, Secret secret) throws Exception {
        ...
        if (connection != null) {
            ...
            if (!StringUtils.isNullOrEmpty(secret.getProductName())) {
                sql.append(" and b.productName like ? ");
                list.add("%" + secret.getProductName() + "%");
            }
            ...
            while (rs.next()) {
                ...
                _secret.setProductName(rs.getString("商品名称"));
    ...
    public Secret queryForOne(Connection connection, String repoId, String providerCode) throws Exception {
        ...
        ResultSet rs = null;
        if (null != connection) {
            String sql="select r.id 键,r.providerCode 供应商编码,r.providerName 供应商名称,p.proPhone 供应商电话,b.productName 商品名称,r.total 总量,r.unit 单位 from smbms_repo r, smbms_provider p, smbms_bill b where r.providerCode = p.proCode and r.id = b.id  and r.providerCode =?  and r.id =? ";
            Object[] params = {providerCode, repoId};
            rs = BaseDao.execute(connection, pstm, rs, sql.toString(), params);
            if (rs.next()) {
                _secret = new Secret();
                _secret.setId(rs.getLong("键"));
    ...
    public int modify(Connection connection, Secret secret) throws Exception {
        ...
        PreparedStatement pstm = null;
        if(null != connection){
            String sql = "UPDATE smbms_repo r, smbms_provider p, smbms_bill b  set  r.total = ? ,r.unit = ? where r.providerCode = p.proCode and r.id = b.id  and r.providerCode =?  and r.id = ?";
